...
build_package() {
  HERE=`pwd`
  cd $PACKAGE_LOCATION
  make clean all CPU_TARGET="$MY_CPU_TARGET" OS_TARGET="$MY_OS_TARGET" OPT="$PACKAGE_OPTIONS" > /dev/null
  cd "$HERE"
}
...
compile() {
  ...
  MANAGER_UNITS="-Fu$MANAGER_LOCATION/utils -Fu$MANAGER_LOCATION/designer -Fu$MANAGER_LOCATION/project -Fu$MANAGER_LOCATION -Fu."
...
#  echo "fpc $CPU $TARGET $MANAGER_OPTIONS $MANAGER_DIRS $PACKAGES_DIRS $LAZARUS_DIRS  $OUTPUTNAME $DEFINEOPTS epidatamanager.lpr"
#  fpc $CPU $TARGET $MANAGER_OPTIONS $MANAGER_DIRS $PACKAGES_DIRS $LAZARUS_DIRS  $OUTPUTNAME $DEFINEOPTS epidatamanager.lpr
  fpc $CPU $TARGET $MANAGER_OPTIONS $MANAGER_DIRS $PACKAGES_DIRS $LAZARUS_DIRS  $OUTPUTNAME $DEFINEOPTS epidatamanager.lpr > /dev/null
